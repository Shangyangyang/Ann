...
public class MyPdfTest {
    ...
    public static void main(String[] args) throws IOException, DocumentException {
        ...
        BaseFont bianhaoBase = new Watermark().getBaseFont("方正大标宋简体");
        Font bianhao = new Font(bianhaoBase, 18); // 其他所有文字字体

        PdfWriter.getInstance(document, new FileOutputStream("D:/MyPdfTestShuiwu.pdf"));
        document.open(); //打开文档

        //------------开始写数据-------------------
        PdfPTable totalTable = new PdfPTable(3);// 建立一个pdf表格
        totalTable.setSpacingBefore(20f);// 设置表格上面空白宽度
        totalTable.setTotalWidth(500);// 设置表格的宽度
        totalTable.setWidthPercentage(100);//设置表格宽度为%100
//        totalTable.setLockedWidth(true);
        // table.getDefaultCell().setBorder(0);//设置表格默认为无边框
        PdfPCell cell = null;

        PdfPTable table1 = new PdfPTable(1);// 建立一个pdf表格
        table1.getDefaultCell().setBorder(0);
        cell = new PdfPCell(new Paragraph("第一个表格", titlefont));
        cell.setBorder(Rectangle.NO_BORDER);
        table1.addCell(cell);

        totalTable.addCell(table1);

        PdfPTable table2 = new PdfPTable(1);// 建立一个pdf表格
        table2.getDefaultCell().setBorder(0);
        cell = new PdfPCell(new Paragraph("第二个表格1", titlefont));
        cell.setBorder(Rectangle.NO_BORDER);
        table2.addCell(cell);
        cell = new PdfPCell(new Paragraph("第二个表格2", titlefont));
        cell.setBorder(Rectangle.NO_BORDER);
        table2.addCell(cell);
        cell = new PdfPCell(new Paragraph("第二个表格3", titlefont));
        cell.setBorder(Rectangle.NO_BORDER);
        table2.addCell(cell);

        totalTable.addCell(table2);

        PdfPTable table3 = new PdfPTable(1);// 建立一个pdf表格
        table3.getDefaultCell().setBorder(0);
        cell = new PdfPCell(new Paragraph("第三个表格1", titlefont));
        cell.setBorder(Rectangle.NO_BORDER);
        table3.addCell(cell);
        cell = new PdfPCell(new Paragraph("第三个表格2", titlefont));
        cell.setBorder(Rectangle.NO_BORDER);
        table3.addCell(cell);
        cell = new PdfPCell(new Paragraph("第三个表格3", titlefont));
        cell.setBorder(Rectangle.NO_BORDER);
        table3.addCell(cell);
        cell = new PdfPCell(new Paragraph("第三个表格4", titlefont));
        cell.setBorder(Rectangle.NO_BORDER);
        table3.addCell(cell);

        totalTable.addCell(table3);
